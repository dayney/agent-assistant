...
Projects are imported by local path. Import records only local application
state; it does not create files in the project. If discovered native Rule files
differ, the review action can ask the installed Codex CLI for a structured
mother-Rule proposal. The proposal is never saved or synchronized until the
