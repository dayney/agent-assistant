...
export function RuleConflictSheet({
  conflict,
  blockedTargets,
  backupConfirmation,
  busy,
  ...
  return (
    <>
        ...
        {conflict && (
          <>
            <div className="dialog-heading">
              <div>
                <p className="eyebrow">Sync Blocked</p>
                <h2 id="conflict-title">{conflict.agent} 存在未同步修改</h2>
              </div>
            </div>
            <p className="dialog-copy">
              同步已经停止。先审查原生差异，再选择导入母版或继续到备份确认。
            </p>
            <code className="dialog-path">
              {conflict.path || conflict.reason || "无目标路径"}
            </code>
            <pre className="diff-view">
              {conflict.diff || conflict.reason || "没有可显示的差异。"}
            </pre>
      ...
      >
        <div className="dialog-heading">
          <div>
            <p className="eyebrow">Backup and Overwrite</p>
            <h2 id="backup-title">备份并覆盖原生 Rule</h2>
          </div>
        </div>
        <p className="dialog-copy">
          此操作会先备份下列原生文件，再以当前母版覆盖。按 Return 不会执行覆盖。
        </p>
        <ul>
