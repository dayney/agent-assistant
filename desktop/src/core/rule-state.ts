...
export function deriveRuleCommandState(
  input: RuleCommandContext,
): RuleCommandState {
  ...
  const previewFresh =
    ...
    input.previewKey === createRulePreviewKey(input);
  ...
  const canSync =
    !input.busy && previewFresh && hasTargets && !hasBlockedTargets;
