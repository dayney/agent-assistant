...
func (plan governanceInit) apply(force bool) error {
	...
	if err := iox.AtomicWrite(configPath, []byte(renderGovernanceConfig(plan.manifest.Agents)), 0o644); err != nil { //nolint:forbidigo // canonical project source, not an Agent destination
		return fmt.Errorf("write %s: %w", configPath, err)
	}
	if err := iox.AtomicWrite(memoryPath, []byte(governance.RenderMemory(plan.profile, plan.manifest.BaselineSHA256)), 0o644); err != nil { //nolint:forbidigo // canonical project source, not an Agent destination
	...
	if err := iox.AtomicWrite(readmePath, []byte(readme), 0o644); err != nil { //nolint:forbidigo // canonical project source, not an Agent destination
	...
	if err := iox.AtomicWrite(manifestPath, manifestBytes, 0o644); err != nil { //nolint:forbidigo // governance state, not an Agent destination
...
func appendGovernanceExcludes(root string, entries []string) error {
	...
	return iox.AtomicWrite(excludePath, []byte(text), 0o644) //nolint:forbidigo // local git metadata, not an Agent destination
}
