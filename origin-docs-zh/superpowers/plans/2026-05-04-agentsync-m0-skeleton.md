...
> **对于代理工作人员：** 所需的子技能：使用 superpowers:subagent-driven-development （推荐）或 superpowers:executing-plans 来逐个任务地实施此计划。步骤使用复选框 (`- [ ]`) 语法进行跟踪。约定（TDD 周期、测试框架、lint、提交格式）在 [`2026-05-04-agentsync-v1.0-overview.md`](2026-05-04-agentsync-v1.0-overview.md#conventions-used-across-all-milestone-plans) 中定义一次。

**目标：** 使用基本框架引导 `agentsync` Go 模块：遵循 `AGENTSYNC_HOME` / `AGENTSYNC_TARGET_ROOT` 的路径解析、原子文件写入 + 文件锁定、slog 设置、规范 TOML 架构 + 加载器、JSON 状态存储、适配器接口 + 注册表、用于测试应用管道的 `NoopAdapter` 以及公开 `init` / 的瘦眼镜蛇 CLI `agent {add,list,remove}` / `doctor` / `verify` / `apply --dry-run`。 CI 绿色。

**架构：** 单个 Go 模块 (`github.com/spxrogers/agentsync`) → `cmd/agentsync/` 处的单个静态二进制文件。内部包分别封装一个关注点。 `paths` 帮助器是生产代码中唯一触及真正的 `$HOME` 的地方；通过 `AGENTSYNC_TARGET_ROOT` 测试重定向。此处定义了适配器接口，但尚未实现真正的适配器 - `NoopAdapter` 让应用管道与空文件操作端到端运行，以便 M1+ 可以插入真正的适配器。

**技术堆栈：** Go 1.22+、`github.com/spf13/cobra` (CLI)、`github.com/pelletier/go-toml/v2` (TOML)、`github.com/spf13/afero`（FS 测试假）、`github.com/gofrs/flock`（文件锁定）、`log/slog`（stdlib）、`golangci-lint`、`goreleaser`（仅配置；在 M7 中发布）。
...
**文件：**
- 创建：`go.mod`、`.gitignore`、`Makefile`、`README.md`、`.golangci.yml`、`.goreleaser.yaml`、`.github/workflows/ci.yml`、`cmd/agentsync/main.go`
...
预期：使用 `module github.com/spxrogers/agentsync` 和 `go 1.22`（或当前工具链）创建 `go.mod`。
...
（注意：forbidigo 的 `_test.go`-only 过滤器是通过 `issues.exclude-rules` invert 配置的；如果 golangci-lint 版本不支持 `pattern:`，请切换到：全局启用 `forbidigo`，然后 `exclude-rules` 和 `path-except: '_test\.go$'` 作为规则。在安装的版本上使用 `golangci-lint run --help` 进行确认。）

- [ ] **步骤 0.6：`.goreleaser.yaml`** （仅配置；管道在 M7 中接线）
...
- [ ] **步骤 0.7：`.github/workflows/ci.yml`**
...
- [ ] **步骤 0.8：`cmd/agentsync/main.go`** （占位符；任务 13 中定义的 cli.Execute）
...
预期：`agentsync dev (commit none, built unknown)`
...
## 任务 1：`internal/paths` — `AGENTSYNC_HOME` / `AGENTSYNC_TARGET_ROOT` 分辨率

**文件：**
- 创建：`internal/paths/paths.go`、`internal/paths/paths_test.go`

生产代码将逻辑名称（“home dir”、“config dir”）映射到文件系统路径的单一阻塞点。通过 `AGENTSYNC_TARGET_ROOT` 测试重定向。
...
预期：`package github.com/spxrogers/agentsync/internal/paths is not in std`。测试文件引用尚不存在的符号（`paths.HomeDir`、`paths.AgentsyncHome`、`paths.MapEnv`）。
...
预期：`ok github.com/spxrogers/agentsync/internal/paths`
...
## 任务 2: `internal/iox` — 原子文件写入

**文件：**
- 创建：`internal/iox/atomic.go`、`internal/iox/atomic_test.go`

两阶段写入：写入`<dest>.agentsync.tmp`，fsync，重命名为final。写入过程中的崩溃会留下旧内容或新内容，而不会出现部分内容。
...
预期：`ok github.com/spxrogers/agentsync/internal/iox`
...
## 任务 3：`internal/iox` — 文件锁定

**文件：**
- 创建：`internal/iox/lock.go`、`internal/iox/lock_test.go`
- 修改：`go.mod`（添加`github.com/gofrs/flock`）

跨进程文件锁定，以便并发 `agentsync apply` 干净地运行序列化。
...
预期：`undefined: iox.AcquireLock`、`undefined: iox.AcquireLockTimeout`
...
## 任务 4：`internal/log` — slog 包装器

**文件：**
- 创建：`internal/log/log.go`、`internal/log/log_test.go`

`log/slog` 周围的微小包装器，因此代码库的其余部分导入一个包，并且 `--verbose` 全局标志翻转级别。
...
## 任务 5：`internal/source` — 规范模式（TOML 结构）

**文件：**
- 创建：`internal/source/schema.go`

定义作为agentsync规范模型的Go结构。没有单独测试此任务 - 由任务 6 的加载器测试执行。
...
## 任务 6: `internal/source` — 加载程序

**文件：**
- 创建：`internal/source/loader.go`、`internal/source/loader_test.go`

遍历 `<home>/` 并生成 `Canonical`。使用 `afero.Fs` 因此测试可以保留在内存中。
...
## 任务 7：`internal/state` — 架构 + 存储

**文件：**
- 创建：`internal/state/schema.go`、`internal/state/store.go`、`internal/state/store_test.go`

`.state/targets.json` 处的单个 JSON 文件。通过 `iox.AtomicWrite` 进行原子操作。
...
## 任务 8：`internal/adapter` — 接口 + 注册表

**文件：**
- 创建：`internal/adapter/adapter.go`、`internal/adapter/registry.go`、`internal/adapter/registry_test.go`

定义 M1 (Claude)、M2 (OpenCode) 和后续里程碑实现的 `Adapter` 接口。注册表只是一张地图。
...
## 任务 9: `internal/adapter/noop` — NoopAdapter

**文件：**
- 创建：`internal/adapter/noop/noop.go`、`internal/adapter/noop/noop_test.go`

在任何真正的适配器存在之前，用于对应用管道进行端到端测试的空适配器。
...
（注意：`source.CanonicalZero()`还不存在；编写的测试无法编译。直接使用`source.Canonical{}`文字以避免为此添加助手。调整：）
...
## 任务 10：`internal/render` — 应用管道

**文件：**
- 创建：`internal/render/pipeline.go`、`internal/render/pipeline_test.go`

编排：加载规范→要求每个已注册+启用的适配器进行渲染→收集FileOps + Skips→`apply`（写入）或返回以进行`--dry-run`打印。

- [ ] **步骤10.1：测试（由NoopAdapter驱动）**
...
## 任务 11：`internal/cli` — 根命令 + 全局标志

**文件：**
- 创建：`internal/cli/root.go`、`internal/cli/root_test.go`、`internal/cli/testhelper_test.go`

cobra root 带有 `--verbose`、`--config`、版本打印。子命令通过 `root.AddCommand()` 附加。

- [ ] **步骤 11.1：添加 cobra 依赖项**
...
（这引用了 `newInitCmd`、`newAgentCmd` 等；当每个命令落地时，占位符前向声明将添加到任务 12-17 中。）

- [ ] **步骤 11.5：存根命令构造函数以便 root.go 编译**

在任务 12-17 中充实每个子命令之前，在各自的文件中添加存根。创建 `internal/cli/init.go`、`agent.go`、`doctor.go`、`verify.go`、`apply.go`，每个都具有最小存根：
...
...以及 `newAgentCmd`、`newDoctorCmd`、`newVerifyCmd`、`newApplyCmd` 的类似存根。他们将在后续任务中被替换。

- [ ] **步骤 11.6：连接 main.go**

更新`cmd/agentsync/main.go`：
...
预期：`--version` 测试通过，`--help` 测试通过（存在子命令存根）。
...
## 任务 12：`agentsync init`

**文件：**
- 修改：`internal/cli/init.go`
- 创建：`internal/cli/init_test.go`

带有空子目录和存根 `agentsync.toml` 的支架 `~/.agentsync/`。
...
## 任务 13：`agentsync agent {add,list,remove}`

**文件：**
- 修改：`internal/cli/agent.go`
- 创建：`internal/cli/agent_test.go`

通过 TOML AST 往返操作 `~/.agentsync/agentsync.toml` 中的 `[agents]` 表（保留注释 + 键顺序）。
...
## 任务 14：`agentsync doctor`

**文件：**
- 修改：`internal/cli/doctor.go`
- 创建：`internal/cli/doctor_test.go`

报告环境+代理安装检测。 M0只检查PATH； M1+ 中提供了更丰富的每个适配器检测功能。
...
## 任务 15：`agentsync verify`

**文件：**
- 修改：`internal/cli/verify.go`
- 创建：`internal/cli/verify_test.go`

Lints `~/.agentsync/`：解析每个 TOML 文件，报告架构违规。 M0表面：“所有文件解析，所有`[agents].<name>`都是已知的适配器。”
...
## 任务 16：`agentsync apply --dry-run`

**文件：**
- 修改：`internal/cli/apply.go`
- 创建：`internal/cli/apply_test.go`

连线源加载器 → render.Plan → 打印计划。 M0 仅支持 `--dry-run`，因为还没有真正的适配器写入任何内容。

- [ ] **步骤 16.1：测试（通过瘦注册表助手使用 NoopAdapter）**

添加 `internal/cli/registry_internal.go` 公开一个用于测试的钩子：
...
**文件：**
- 创建：`internal/cli/integration_test.go`

端到端：`init → agent add claude → agent add opencode → verify → apply --dry-run` 全面返回 0。
...
PR 上的 CI 应变为绿色：linux/macos/windows 上的 `test` 作业、`lint` 作业、`goreleaser-snapshot` 作业。
...
Linux + macos + windows 上的 CI 绿色，适用于 `test` (race) 和 `lint`。 `goreleaser` 快照已成功构建。

该框架支持 M1（Claude 适配器）、M2（OpenCode 适配器）、M3（漂移）、M4（插件）、M5（项目）、M6（秘密）、M7（抛光/发布），无需架构更改。