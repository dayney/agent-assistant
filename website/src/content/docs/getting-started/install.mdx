...
Desktop can be built from source today. Signed Desktop downloads are not
advertised until the separate signing and notarization release gate is enabled.
The repository's
[machine-readable policy](https://github.com/spxrogers/agentsync/blob/main/platform-support.json)
is the source of truth for versions, architectures, CI runners, and cadence.
...
You'll need Go >= 1.26.5 (the version pinned in `go.mod`).
